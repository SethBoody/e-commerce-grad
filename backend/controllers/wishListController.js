...
const addItemToWishList = asyncWrapper(async (req, res, next) => {
  ...
  if (!userId || !productId) {
    return next(
      createCustomError(
        'userId and productId are required in the request body',
        400
      )
    );
  }
...
const deleteWishListProducts = asyncWrapper(async (req, res, next) => {
  ...
  if (!userId) {
    return next(
      createCustomError('userId is required in the request body', 400)
    );
  }
...
const getWishListItems = asyncWrapper(async (req, res, next) => {
  ...
  if (!wishList) {
    return next(
      createCustomError(`No wish list with id: ${wishListId} is found`, 404)
    );
  }
